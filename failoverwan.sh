...
DEF_INTERFACE=$1 # first ISP interface
FAILOVER_INTERFACE=$2 # second ISP interface
...
while [[ -z "$(route | grep default | awk '{print $8}')" ]]
do
  sleep 1
done
...
while true; do
              internal_checking_ip=$(ifconfig $DEF_INTERFACE | grep -o 'destination [0-9][0-9.]*') # get default gw for main ISP
              external_checking_ip=$(ifconfig $FAILOVER_INTERFACE | grep -o 'destination [0-9][0-9.]*') # get default gw for second ISP
              checking_ip=${internal_checking_ip#destination } # get clean gw IP for main ISP
              external_checking_ip=${external_checking_ip#destination } # get clean gw IP for second ISP
              ...
              if [[ -z "$checking_ip" ]]; then # if var-indicator clean - switch ISP
